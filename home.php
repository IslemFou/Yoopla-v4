...
<section>
	...
	<!-- search bar -->
	<form method="POST" action="#searchResults" title="Formulaire de recherche">
	...
	<div class="container rounded-3 p-3" id="searchResults">
		<?php
	...
	<div class="d-flex justify-content-around flex-wrap container">
		...
		if (empty($limitedEvents)) {
			...
			foreach ($limitedEvents as $event) :
				...
				$event_description = ucfirst(strtolower(substr($event['description'] ?? 'Pas de description.', 0, 90) . '...')); // Limit description to 100 characters and add ellipsis (...) if truncated. $event['Description'] ?? 'Pas de description;
